...
from .new_app_ws_program import NewAppWsProgram
...
async def test_wsprogram_start_creates_tasks():
    ...
    ws_program.ws_server.start = ws_loop_mock
    ws_program.redis_consumer.start = redis_loop_mock

    # Act
    await ws_program.start()

    # Assert start_loop was called
    ws_program.ws_server.start.assert_awaited_once()
    ws_program.redis_consumer.start.assert_awaited_once()
...
async def test_wsprogram_start_cancels_pending_task():
    ...
    ws_program.ws_server.start = AsyncMock()
    ws_program.redis_consumer.start = AsyncMock()
